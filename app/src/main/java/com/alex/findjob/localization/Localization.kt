package com.alex.findjob.localization
...
internal val russianLocale = Locale("ru")

internal val defaultLocalization: Localization = Localization(russianLocale)
...
fun registerSupportedLocales(vararg locales: Locale): Set<Locale> {
    locales.filter { it != russianLocale }
        .forEach {
            if (supportedLocales.add(it)) {
                registerLocalizationForLocale(it)
            }
        }
    return supportedLocales + russianLocale
}
...
fun Translatable(
    ...
    for ((locale, value) in localeToValue.entries) {
        localizationMap[locale]?.let { it.strings[id] = value }
    }
